...
const int FONT_SIZE = MAX_PATH - 1;									//フォントサイズ。
...
void SoulUI::Update()
{
	...
	case SoulUI::SoulUI_Default:
		SoulUIDefault();		//通常状態。(非表示）
		break;
	case SoulUI::SoulUI_GetMove:
		SoulUIGetMove();		//魂獲得時所持バーの出現処理。
		break;
	case SoulUI::SoulUI_SoulCount:
		SoulCount();			//魂の数をカウントする。
		break;
	case SoulUI::SoulUI_FontBoundMove:
		FontBoundMove();		//魂獲得時フォントのカウントをバウンドさせる処理。
		break;
	case SoulUI::SoulUI_Indicate:
		SoulUIIndicate();		//一定時間だけ表示状態のまま保つ。
		break;
	case SoulUI::SoulUI_Return:
		SoulUIReturn();			//元の状態に戻す。
		break;
	}
}
